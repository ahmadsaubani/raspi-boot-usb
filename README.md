...
# Mount writable partition with sudo mount /dev/sda2 /mnt/principal
...
if [ -d /media/*/principal ]; then
   echo "Safety check:  automount detected at /media/*/principal.  Please unmount the automount in File Explorer or with sudo umount /media/$LOGNAME/system-boot."
   exit 1
fi

# Find the "principal" root filesystem mount
if [ -d /mnt/principal ] && [ -d /mnt/principal/usr/lib/u-boot ] ; then
    mntWritable='/mnt/principal'
else
    echo "The partition 'writable' was not found in /mnt/principal.  Make sure you have mounted your USB mass storage device (ex: sudo mount /dev/sda2 /mnt/principal)."
    exit 1
fi
